...
class Validator {
  ...
  ctx (_ctx={}, context, clone) {
    ...
    let addError = (e) => {
      let oldErrs = ctx.fleek.validation.errors;
      ctx.fleek.validation.passed = false;
      ctx.fleek.validation.failed = true;
      if (e.errors) {
        ctx.fleek.validation.errors = oldErrs.concat(e.errors);
      } else if (this.isErrorList(e)) {
        ctx.fleek.validation.errors = oldErrs.concat(e);
      }
